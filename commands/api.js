  ...
  async execute(message, args, bot) {
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(data.exhausted ? 0xFF0000 : data.remaining > 10 ? 0x00FF00 : 0xFFFF00)
        .addFields(
          { name: '📊 Limit', value: `${data.limit.toLocaleString()}`, inline: true },
          { name: '✅ Used', value: `${data.used.toLocaleString()}`, inline: true },
          { name: '🎯 Remaining', value: `${data.remaining.toLocaleString()}`, inline: true },
          { name: '⚠️ Status', value: data.exhausted ? '❌ **EXHAUSTED**' : '✅ Active', inline: false }
        )
        .addFields(
          { name: '📅 Created', value: data.created_at ? new Date(data.created_at).toLocaleString('en-IN') : 'Unknown', inline: true }
        )
        .setFooter({ text: `AI API SYSTEM BY YAKUZA & GANG`, icon_url: 'https://cdn.discordapp.com/attachments/1455235201910444238/1455790002268143686/logo.png' })
        .setTimestamp();

      if (data.remaining <= 10 && !data.exhausted) {
      ...
      if (error.code === 'ECONNABORTED') {
        errorMsg = '⏱️ API timeout (5s)';
      } else if (error.response?.status === 401) {
        errorMsg = '❌ Invalid API key';
      } else if (error.response?.status === 404) {
        errorMsg = '❌ /balance endpoint not found';
